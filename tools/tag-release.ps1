...
# Creates and pushes a vX.Y.Z release tag for delphi-incver.
...
#   - $script:ToolVersion in delphi-incver.ps1 matches the Version argument
...
Write-Host ""
Write-Host "delphi-incver  tag-release" -ForegroundColor White
...
if ($scriptContent -notmatch '\$script:ToolVersion\s*=\s*''([^'']+)''') {
  Fail "Could not find '`$script:ToolVersion = ''...''' in delphi-incver.ps1."
}

$scriptVersion = $Matches[1]
if ($scriptVersion -ne $Version) {
  Fail "`$script:ToolVersion in delphi-incver.ps1 is '$scriptVersion' but -Version arg is '$Version'.`n       Update `$script:ToolVersion in the script and commit before tagging."
}
